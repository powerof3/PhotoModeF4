...
namespace ImGui
{
		...
		void ResetIndex()
		{
			...
			if constexpr (std::is_same_v<T, RE::TESWeather>) {
				if (auto currentWeather = RE::Sky::GetSingleton()->currentWeather) {
					if (const auto it = edidForms.find(currentWeather->GetFormEditorID()); it != edidForms.end()) {
	...
	public:
			...
			if (modNameForms.empty()) {
				if constexpr (std::is_same_v<T, RE::TESIdleForm>) {
					...
					for (const auto& form : RE::TESDataHandler::GetSingleton()->GetFormArray<T>()) {
						if (form) {
							AddForm(form->GetFormEditorID(), form);
						}
					}
				}
			}
		...
		void GetFormResultFromCombo(std::function<void(T*)> a_func, RE::Actor* a_actor = nullptr, bool a_showTitle = true)
		{
			T* formResult;
			...
			ImGui::BeginGroup();
			{
				...
				ImGui::PushStyleColor(ImGuiCol_Header, GetColorU32(ImGuiCol_Text));

				ImGui::PushID(name.c_str());
				ImGui::PushMultiItemsWidths(2, ImGui::GetContentRegionAvail().x - ImGui::GetStyle().ItemSpacing.x);
